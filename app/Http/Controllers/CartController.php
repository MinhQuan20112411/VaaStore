    ...
    public function addToCart(Request $request)
    {
        ...
        if (isset($cartItems[$request->input('product_id')])) {
            ...
            $cartItems[$request->input('product_id')] = [
                ...
                'image' => asset('public/images/' . $product->product_img),
                'quantity' => 1
            ];
        }
        ...
        return response()->json([
            'status' => 'success',
            'cart_count' => count($cartItems), // Số lượng sản phẩm trong giỏ hàng
            'cart_items' => array_values($cartItems) // Danh sách sản phẩm trong giỏ hàng
        ]);
    }
